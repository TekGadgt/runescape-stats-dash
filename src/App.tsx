...
function App() {
  ...
  const getXPProgress = (level: number, xp: number) => {
    // Max level (99) shows full bar
    if (level >= 99) return 100;

    // XP table: index 0 corresponds to level 1 (0 XP), index 1 -> level 2 requirement, etc.
    ...
    if (level < 1) return 0; // Guard against invalid levels

    const currentLevelIndex = level - 1; // array index for current level requirement
    const nextLevelIndex = level; // array index for next level requirement

    const currentLevelXP = xpTable[currentLevelIndex] ?? 0;
    const nextLevelXP = xpTable[nextLevelIndex] ?? currentLevelXP; // if missing, avoid division by zero

    const delta = nextLevelXP - currentLevelXP;
    if (delta <= 0) return 100; // should not happen except at cap

    const progress = ((xp - currentLevelXP) / delta) * 100;
                  ...
                  <div key={skill.id} className="p-3 rounded-lg border bg-card/50 hover:bg-card transition-colors">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex justify-between text-xs text-muted-foreground">
                        ...
                        <span>Rank: {formatNumber(skill.rank)}</span>
                      </div>
                    </div>
                  </div>
                );
